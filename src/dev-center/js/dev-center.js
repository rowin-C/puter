...
if (URLParams.has("puter.port") && URLParams.get("puter.port")) {
  static_hosting_domain =
    static_hosting_domain + `:` + html_encode(URLParams.get("puter.port"));
}

// protocol
let protocol = "https";
if (
  URLParams.has("puter.protocol") &&
  URLParams.get("puter.protocol") === "http"
)
  protocol = "http";
  ...
  setTimeout(async function() {
    ...
    if (domain === "puter.com") {
      puter.apps.getDeveloperProfile(async function(dev_profile) {
        ...
        if (
          dev_profile.approved_for_incentive_program &&
          !dev_profile.joined_incentive_program
        ) {
        ...
        if (
          ...
          puter.kv.get("earn-money-c2a-closed").then((value) => {
            if (value?.result || value === true || value === "true") return;
  ...
  // ---------------------------------------------------------------
    ...
    setTimeout(function() {
      puter.apps.getDeveloperProfile(function(dev_profile) {
        ...
        $("#loading").hide();
        if (activeTab === "payout-method") $("#tab-payout-method").show();
      });
    }, 1000);
  }
});
...
$(document).on("click", ".create-an-app-btn", async function(e) {
  let title = await puter.ui.prompt(
    "Please enter a title for your app:",
    "My Awesome App"
  );
  ...
  puter.apps
    ...
    .then(async (app) => {
      ...
      puter.apps
        .update(app.name, {
          title: title,
          indexURL: source_path
            ? protocol + `://${subdomain}.` + static_hosting_domain
      ...
      // ----------------------------------------------------
      puter.fs.upload(new File([], app.title), `/${authUsername}/Desktop`, {
        name: app.title,
        dedupeName: true,
        overwrite: false,
        appUID: app.uid,
...
$(document).on("click", ".delete-app", async function(e) {
  ...
  if (app_data.metadata?.locked) {
    puter.ui.alert(
      `<strong>${app_data.title}</strong> is locked and cannot be deleted.`,
      [
        {
          label: "Ok",
        },
      ...
      .then(async (app) => {
        setTimeout(
          () => {
            ...
            $(`.app-card[data-uid="${app_uid}"]`).fadeOut(
              200,
      ...
      if (e.detail?.items?.length > 0) {
        ...
        if (items.length === 1 && !items[0].isDirectory) {
          if (items[0].name.toLowerCase() === "index.html") {
            ...
            puter.ui.alert(
              `You need to have an index.html file in your deployment.`,
              [
                {
                  label: "Ok",
                },
        ...
        // ----------------------------------------------------
          ...
          for (let child of children) {
            if (child.name === "index.html") {
              ...
              if (children.length === 1) rootItems = children[0].name;
...
$(document).on("click", ".edit-app-save-btn", async function(e) {
  ...
  //validation
  if (title === "") error = `<strong>Title</strong> is required.`;
  else if (title.length > 60)
    error = `<strong>Title</strong> cannot be longer than ${60}.`;
  else if (name === "") error = `<strong>Name</strong> is required.`;
  else if (name.length > 60)
    error = `<strong>Name</strong> cannot be longer than ${60}.`;
  else if (index_url === "") error = `<strong>Index URL</strong> is required.`;
    ...
    !index_url.toLowerCase().startsWith("https://") &&
    !index_url.toLowerCase().startsWith("http://")
  )
  ...
  // download icon from URL
    ...
    if (icon_base64) {
      ...
      // make sure icon is an image
        ...
        !icon.startsWith("data:application/octet-stream")
      )
...
window.deploy = async function(app, items) {
  ...
  if (
    ...
    if (stat.is_dir) {
      ...
      if (files.length > 0) {
        for (let file of files) {
          // perform copy
          await puter.fs.copy(file.path, appdata_dir.path, { overwrite: true });
          // update progress
          $(".deploy-percent").text(
            `(${Math.round((files.indexOf(file) / files.length) * 100)}%)`
          );
        }
      }
    }
    ...
    // --------------------------------------------------------------------
      ...
      // copy the 'files' to the app directory
      await puter.fs.copy(items, appdata_dir.path, { overwrite: true });
    }
    ...
    puter.hosting.create(hostname, appdata_dir.path).then(async (res) => {
      ...
      $("#edit-app-index-url").val(
        protocol + `://${hostname}.` + static_hosting_domain
      );
  ...
  // --------------------------------------------------------------------
    ...
    for (let item of items) {
      ...
      // update progress
      $(".deploy-percent").text(
        `(${Math.round((items.indexOf(item) / items.length) * 100)}%)`
      );
    }
...
function setRootDirTree(tree, items) {
  ...
  if (
    rootKeys.length === 1 &&
    typeof tree[rootKeys[0]] === "object" &&
    Object.keys(tree[rootKeys[0]]).length > 0
  ) {
    let newItems = [];
    for (let item of items) {
      if (item.fullPath)
        item.finalPath = item.fullPath.replace(rootKeys[0], "");
      else if (item.path) item.path = item.path.replace(rootKeys[0], "");
      else item.filepath = item.filepath.replace(rootKeys[0], "");
...
$(document).on("click", ".insta-deploy-to-new-app", async function(e) {
  ...
  } else $(".insta-deploy-modal").get(0).showModal();
...
$(document).on("click", ".insta-deploy-to-existing-app", function(e) {
  ...
  $(".insta-deploy-existing-app-list").html(
    `<div style="margin: 100px auto 10px auto; width: 40px; height:40px;">${loading_spinner}</div>`
  );
...
$(document).on("click", ".add-app-to-desktop", function(e) {
  ...
  puter.fs
    ...
    .then(async (uploaded) => {
      puter.ui.alert(
        `<strong>${app_title}</strong> shortcut has been added to your desktop.`,
        [
          {
            label: "Ok",
            type: "primary",
